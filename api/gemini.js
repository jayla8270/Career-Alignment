...
export default async function handler(req, res) {
    ...
    try {
        ...
        const { lang } = payload;
        const systemInstruction = SYSTEM_PROMPT.replace('[LANG]', getLangString(lang));

        let responseText = "{}";

        switch (action) {
            case 'structureExperience': {
                const { rawText, fileData } = payload;
                const parts = [{
                    text: `
        ...
        ` }];
            ...
            case 'performFitCheck': {
                const { experience, jd } = payload;
                const response = await ai.models.generateContent({
                    model: MODEL_NAME,
                    contents: `
            [LANGUAGE]: ${getLangString(lang)}.
          ...
          `,
            ...
            case 'generateResumeDraft': {
                const { experience, jd, isFinal } = payload;
          ...
          `,
            ...
            case 'refineResume': {
                const { currentResume, experience, jd, feedback } = payload;
                const response = await ai.models.generateContent({
                    model: MODEL_NAME,
                    contents: `
            [LANGUAGE]: ${getLangString(lang)}.
